...
export class UI {
  ...
  // ---------------------------------------------------------------- battle
  private startBattle(level: number, training: boolean): void {
    unlockAudio();
    ...
    this.banner(training ? 'TRAINING' : enemy.boss ? 'BOSS' : `LEVEL ${level}`, training ? 'Draw glyphs to test your spells' : enemy.name);
  ...
  private loop(now: number): void {
    ...
    a.update(dt, b);
    this.updateHud(dt);
    if (b.over && b.overT > 1.8) this.endBattle();
  }
  ...
  private buildHud(enemy: EnemyDef, loadout: SpellDef[]): void {
    ...
    if (this.training) {
      sparBtn = btn('Dummy: passive', 'small ghost', () => {
        ...
        this.battle.sparring = !this.battle.sparring;
        sparBtn.textContent = this.battle.sparring ? 'Dummy: attacks' : 'Dummy: passive';
      });
  ...
  private dodge(dir: -1 | 1): void {
    ...
    if (this.battle.dodge(dir)) sfx.dodge();
  }
  ...
  private handleEvents(events: BattleEvent[]): void {
    for (const ev of events) {
      switch (ev.type) {
        case 'cast':
          if (ev.who === 'player' && ev.spell) { sfx.cast(0.8 + (ev.spell.cost / 65) * 0.6); const chip = this.hud.chips.get(ev.spell.id); if (chip) { chip.classList.add('flash'); setTimeout(() => chip.classList.remove('flash'), 220); } }
          else sfx.cast(0.6);
          break;
        ...
        case 'damage':
          ...
          else { if (!ev.dot) sfx.hitPlayer(); this.popup(`-${fmt(ev.amount)}`, 'player', ev.dot ? 'small' : '', ev.dot ? ev.color : '#ff6a6a'); if (ev.absorbed) { sfx.shield(); this.popup(`${fmt(ev.absorbed)} blocked`, 'player', 'small', '#6ea8ff'); } }
          break;
  ...
  private endBattle(): void {
    ...
    if (outcome === 'win') {
      this.save.wins++;
      if (level > this.save.best) { this.save.best = level; void sdk.sendScore(level); }
      this.save.level = Math.min(MAX_LEVEL, Math.max(this.save.level, level + 1));
      this.pickedLevel = Math.min(MAX_LEVEL, level + 1);
    } else {
      this.save.losses++;
  ...
  private confirmForfeit(): void {
    this.openOverlay((box, close) => {
      ...
      row.append(btn('Keep fighting', 'gold', close), btn('Forfeit', 'ghost', () => {
        close();
        if (this.battle) { this.battle.player.hp = 0; this.battle.over = 'lose'; this.battle.overT = 5; }
      }));
      box.append(row);
    });
  }
  ...
  private renderShop(): void {
    ...
    const names: [ShopTab, string][] = [['spells', 'Spells'], ['upgrades', 'Upgrades'], ['gear', 'Gear'], ['coins', 'Coins'], ['training', 'Training']];
  ...
  private renderCoins(body: HTMLElement): void {
    ...
    const b = btn('WATCH AD', 'gold big', async () => {
      b.disabled = true;
      const ok = await sdk.requestRewardedAd('coins');
      if (ok) {
